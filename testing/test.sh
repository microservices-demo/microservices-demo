...
Runs all the shell scripts in the specified directory(s) and asserts that they return an exit status of 0. For example:
 $(basename $0) unit component

It will return an exit code > 0 if the tests do not pass.
...
verbose "Adding assertion methods"
...
for dir in $@ ; do
	...
	# Removing --tag, fails for 'unit' tests
	...
	assert_raises "docker run --rm --name $dir-test -v /var/run/docker.sock:/var/run/docker.sock -v $CODE_DIR:$CODE_DIR -w $CODE_DIR/testing test-container sh -c 'export PYTHONPATH=\$PYTHONPATH:\$PWD/testing ; python $f'"
    done
